        ...
        <div className="grid grid-cols-1 gap-x-8  lg:grid-cols-4">
          {!isSearchPage && !isBrandsPage ? (
            ...
            <div className="hidden lg:block space-y-1.5">
              <Typography bold={"semibold"} size={"lg"}>Category</Typography>
              {categoryData.categories.map((cat_data: any, indx: number) => (
                <div className="flex justify-between  ">
                  ...
                  >
                    <Typography size={"linkText"} bold={"light"}>{cat_data.name}</Typography> 
                  </Link>
                  <div className=" p-2 py-0.5 rounded-full bg-[#f8f8f8] text-xs flex item-center font-[300] leading-normal">{cat_data.count}</div>
                </div>
              ))}
            </div>
          ) : null}
