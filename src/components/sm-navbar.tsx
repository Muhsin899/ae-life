...
const SmNavbar = ({ setLanguageModal }: { setLanguageModal: any }) => {
  const { t } = useLanguage();
  const { data: session } = useSession();
  const { locationOnClickHandle, AddressDataIndex, searchButtonOnClick } =
    useModal();
    ...
    <div className="navbar-container bg-white  md:hidden block ">
      ...
      <div className="container-page">
        <div className="flex  items-center py-1 space-x-3 rtl:space-x-reverse">
          ...
          <Input
            ...
            inputClassName="placeholder:text-xs placeholder:text-[ #002579]"
            className="font-[300]"
            iconLeft={
              <Icon
                type="searchIcon"
      ...
      <div className="flex py-0.5  bg-life text-white  justify-between items-center container-page">
        ...
        <Button
          ...
          iconLeft={
            <Icon type="locationPinIcon" sizes={"xs"} className="!w-2 !h-3 mr-1" />
          }
          variant="white"
          size={"xs"}
          rounded={"sm"}
