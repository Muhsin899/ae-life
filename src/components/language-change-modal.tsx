    ...
    <div>
      {countries.map((contr: any) => (
        <Button
          ...
          className="border  border-muted first:rounded-b-none last:rounded-t-none even:border-t-0 py-3 !px-5 w-full"
        >
  ...
  return (
    <>
      <ModalContainer showModal={modalState} setCloseModal={closeModal}>
        ...
        {IsCountryChangeClicked ? <div>{countryProps}</div> : null}

        {IsLanguageChangeClicked ? <div>{languageProps}</div> : null}
      </ModalContainer>
    </>
  );
};
