  ...
  return (
    <>
      <span
        ...
        } space-x-1 rtl:space-x-reverse opacity-90 sm:px-[7px] px-[4px]   rounded shadow-xl text-white items-center sm:py-1 py-[3px] ${reviewColor(
          productRating
        )}`}
      >
        <Icon
          sizes={"xs"}
          type="starIcon"
          className="fill-white my-auto "
        />

        <Typography size={"xs"} className="!leading-[1]">
          {productRating}
        </Typography>
      </span>
    </>
...
const ProductWishList = ({
  ...
  return (
    <>
      <span
        className="absolute active:scale-75 scale-100 duration-300 right-2 bottom-2 bg-white/70 backdrop-blur-sm sm:p-1.5 p-1 rounded-full cursor-pointer z-[5] drop-shadow-md"
        onClick={() => {
          wishListOnCLick();
        }}
      >
...
const ProductOfferBadge = ({ offersData }: { offersData: any }) => {
  return offersData ? (
    <div className="absolute left-2 top-2 bg-red-500 rounded-full text-white sm:text-sm text-[10px] p-[5px] shadow-lg text-center overflow-hidden leading-3  label-circle sm:w-[3rem] sm:h-[3rem] w-[2.2rem] h-[2.2rem]">
      {offersData.value ? (
  ...
  return proLabelData ? (
    ...
    >
      ...
      <Typography
        size={"xs"}
        lineClamp={"one"}
        className="sm:leading-[1.5rem] leading-normal"
      >
