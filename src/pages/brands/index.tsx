...
export default function BrandsPage({
  ...
  return (
    <div className="mx-auto max-w-[1450px] px-[10px] ">
      ...
      <div
        className=" grid xl:grid-cols-5 sm:grid-cols-4 grid-cols-3  gap-3  mx-auto"
        id="brands-section"
      >
        {brandsData.map((bd: any) => (
          <div className="grid-flow-row mb-5">
            {" "}
            <Link
              href={`/brand/${bd.slug}`}
              className={`flex flex-col mr-5 group/brandMenu`}
            >
              <Image
                className="mx-auto rounded-full border border-white bg-white shadow-md"
                width={120}
                height={120}
                src={bd.images.logo}
                alt=""
              />
              <h5 className="text-center mt-3 xl:text-base md:text-sm text-xs group-hover/brandMenu:text-blue-500 transition-colors duration-300 text-black">
                {bd.name}
              </h5>
            </Link>
          </div>
        ...
        <div className="w-fit flex mx-auto">
          {alphabets.map((alpha: string) => (
            <button
              ...
              className="text-[#39f] font-extrabold scale-100 hover:scale-150 duration-200 transition-all ease-in mr-4 text-3xl "
            >
              {alpha}
            </button>
          ))}
      ...
      {alphabets.map((alpha) => (
        <div className="space-y-3" id={`${alpha}`}>
          <h1 className="underline-offset-2 underline font-bold text-3xl py-2 sticky md:top-40 top-12 z-30">
            {alpha}
          </h1>
            ...
            {alphaBrands[alpha].map((brand: any) => (
              <Link href={`/brand/${brand.slug}`}>
                <Image
                  ...
                  width={180}
                  alt={brand.name}
                  className=" rounded-full translate-y-0 duration-500 mx-auto hover:-translate-y-4"
                />
